...
11. 用户可暂停、恢复和停止实验，并查看当前地图内各智驾等级车辆数、平均速度、排队长度和组件健康。
...
- 系统默认使用浅色主题，并允许在设置中切换深色主题；浅色地图使用`#f1f3f9`背景、白色道路和
  `#fcea87`车道分割线，不显示从车道中心线派生的居中虚线；
- 深色地图沿用原有配色，同样不显示从车道中心线派生的居中虚线；
...
- 用户在“仿真配置”点击“开始仿真”，实验创建成功后自动进入实时监控并按
  `prepare → start` 状态机启动；
- 工作区总览的“区域预览”复用MapLibre/deck.gl显示首个有效SUMO场景包的标准路网，
  不再显示静态占位内容；
